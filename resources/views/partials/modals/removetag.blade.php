    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body text-center">
                @if($tagGroups->where('tag_slug', $tag->slug)->count() > 0)
                    Op dit moment zijn er 
                    <a href="#" data-toggle="tooltip" title="" data-original-title="{{ $allTools->whereIn('slug', $tagGroups->where('tag_slug', $tag->slug)->pluck('tool_slug'))->pluck('name')->implode(', ') }}">
                        {{$tagGroups->where('tag_slug', $tag->slug)->count()}} tool(s)
                    </a> met deze tag,<br>
