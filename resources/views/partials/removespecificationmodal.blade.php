    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                {{ Form::model($specification, array('route' => array('specifications.destroy', $specification->slug), 'method' => 'DELETE')) }}
                    @if($specificationGroups->where('specification_slug', $specification->slug)->count() > 0)
                        {{ Form::submit('Verwijderen', array('class' => 'btn btn-danger btn-avans', 'disabled' => 'disabled')) }}
                    @else
                        {{ Form::submit('Verwijderen', array('class' => 'btn btn-danger btn-avans')) }}
                    @endif
                {{ Form::close() }}
            </div>
        </div>
